...
import { updateCardListing } from "./actions";
import {
  RAW_CONDITIONS,
  SEALED_CONDITIONS,
  GRADING_COMPANIES,
...
export default function EditCardListingClient({
  listingId,
  listing,
}: {
  listingId: string;
  ...
  const { card } = listing;
  const cardIsSealed = SEALED_PRODUCT_TYPES.has(card.product_type);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const result = await updateCardListing(listingId, fd);
    if (result?.error) {
      ...
      <div className={sectionCls}>
        ...
        <div className="flex gap-4 items-start">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">{card.set_name} · #{card.card_number} · {PRODUCT_TYPE_LABELS[card.product_type]}</p>
            <p className="text-xs text-gray-400 mt-1">Card cannot be changed. Cancel and create a new listing if needed.</p>
          </div>
        </div>
      </div>
