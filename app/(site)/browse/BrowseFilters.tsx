...
const inputCls =
  "text-sm border-2 border-black px-3 py-1.5 bg-white text-black font-medium " +
  "focus:outline-none w-24 placeholder:text-gray-700";
...
export default function BrowseFilters() {
  ...
  function switchMode(newMode: string) {
    ...
    if (newMode !== "items") next.set("mode", newMode);
    router.push(`${pathname}${next.toString() ? `?${next.toString()}` : ""}`);
  }

  function handleItemSearch(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    const q = (e.currentTarget.elements.namedItem("q") as HTMLInputElement).value.trim();
    update("q", q);
  }

  function handleUserSearch(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    const userq = (e.currentTarget.elements.namedItem("userq") as HTMLInputElement).value.trim();
    const next = new URLSearchParams();
    next.set("mode", "users");
    if (userq) next.set("userq", userq);
    router.push(`${pathname}?${next.toString()}`);
  }
  ...
  const hasFilters = mode === "users"
    ? !!userq
    : !!(q || type || condition || product || year || min || max || (sort && sort !== "newest"));

  return (
    <div className="space-y-3">
      {/* Mode toggle */}
      <div className="flex gap-0 border-2 border-black w-fit">
        {(["items", "users"] as const).map((m) => (
          <button
            key={m}
          ...
          >
            {m === "items" ? "Browse items" : "Find users"}
          </button>
        ))}
      </div>
        ...
        <>
          <form onSubmit={handleUserSearch} className="flex gap-2">
            <input
              ...
              className="flex-1 px-3 py-2 border-2 border-black text-sm text-black font-medium focus:outline-none placeholder:text-gray-700"
            />
          ...
          {hasFilters && (
            <button
              onClick={() => router.push(`${pathname}?mode=users`)}
              className="text-xs font-bold text-black border-2 border-black px-3 py-1.5 hover:bg-black hover:text-white transition-colors"
            >
              ✕ Clear search
            </button>
          )}
